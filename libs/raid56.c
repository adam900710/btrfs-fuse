...
/* Cross-ported from btrfs-progs, which is further cross-ported from kernel. */
...
static int get_raid56_devs_min(u64 profile)
{
	/* Btrfs supports 2 device RAID5, even though it's the same as RAID1 */
	if (profile & BTRFS_BLOCK_GROUP_RAID5)
		return 2;

	/* The same applies to RAID6, thus 3 devices RAID6 is also allowed */
		 ...
		 int dest2, void **data)
{
	...
	min_devs = get_raid56_devs_min(profile);
	if (min_devs < 0)
		return min_devs;
	if (nr_devs < min_devs)
		return -EINVAL;
